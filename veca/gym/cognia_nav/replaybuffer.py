...
class ReplayBuffer():
    def __init__(self, BUFFER_LENGTH, env):
        ...
        self.data = {
            ...
            'pos0': np.zeros([self.BUFFER_LENGTH, NUM_AGENTS, 2, 3]),
            'pos1': np.zeros([self.BUFFER_LENGTH, NUM_AGENTS, 2, 3]),
            'objimg': np.zeros([self.BUFFER_LENGTH, NUM_AGENTS, 2]),
            'obj': np.zeros([self.BUFFER_LENGTH, NUM_AGENTS, 3]),
        ...
        if RNN:
            ...
            self.data['state1'] = np.zeros([self.BUFFER_LENGTH, NUM_AGENTS, NUM_TIME, STATE_LNEGTH])
    ...
    def add_replay(self, img0, img1, wav0, wav1, Rwav0, Rwav1, touch0, touch1, Rtouch0, Rtouch1, action, helper_reward, raw_reward, pos0, pos1, done, objimg, objwav, objtouch, state0 = None, state1 = None):
        ...
        self.data['obj'][ind] = objwav.copy()
        self.data['objtouch'][ind] = objtouch.copy()
        if RNN:
            self.data['state0'] = np.zeros([self.BUFFER_LENGTH, NUM_AGENTS, NUM_TIME, STATE_LENGTH])
            self.data['state1'] = np.zeros([slef.BUFFER_LENGTH, NUM_AGENTS, NUM_TIME, STATE_LENGTH])
