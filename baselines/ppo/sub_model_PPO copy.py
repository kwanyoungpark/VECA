...
class Model():
    def __init__(self, env, sess, gamma, lambda_coeff, timestep, num_batches, encoder, encoder0, obs_sample, tag): 
        ...
        with tf.variable_scope(name):
            self.placeholders_inf = {k : tf.placeholder(tf.float32, shape = v) for k,v in obshape_cur.items()} # Cur

            self.placeholders = {k : tf.placeholder(tf.float32, shape = (self.timestep,) + v) for k,v in tensor_shapes.items()} # Prev

            self.memory = {k : tf.get_variable("mem/" + k,tf.float32, shape = (num_batches, BATCH_SIZE) + v[1:], trainable = False) for k,v in tensor_shapes.items()}
           
            self.batch = {k : tf.get_variable(k,tf.float32, shape = (BATCH_SIZE,) + v[1:], trainable = False) for k,v in tensor_shapes.items()}
    ...
    def make_batch(self, data):
        ...
        obs_infer = {self.placeholders_inf[key]:data[key][-1]  for key in self.placeholders_inf.keys()}
        ...
        myu, sigma, V0 = self._batch_process(self.sess, self.load_op, ops = [self.oldA, self.V0], feed_dict= None )
        ...
        dict_all.update({
            self.placeholders["myu"]:myu, self.placeholders["sigma"]:sigma, self.placeholders["oldV0"]: V0, 
            self.placeholders["advs"]: (advs - advs.mean()) / (advs.std() + 1e-8), 
            self.placeholders["Vtarget"]: Vtarget, 
