...
from tasks.MazeNav.constants import NUM_OBJS
...
class ReplayBuffer():
    def __init__(self, BUFFER_LENGTH, env):
        ...
        self.data = {
            ...
            'obj': np.zeros([self.BUFFER_LENGTH, NUM_AGENTS, NUM_OBJS]),
        ...
        if RNN:
            self.data['state0'] = np.zeros([self.BUFFER_LENGTH, NUM_AGENTS, STATE_LENGTH])
            self.data['state1'] = np.zeros([self.BUFFER_LENGTH, NUM_AGENTS, STATE_LENGTH])
    ...
    def add_replay(self, img0, img1, action, helper_reward, raw_reward, obj, done, state0 = None, state1 = None):
